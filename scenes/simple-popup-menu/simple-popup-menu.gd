...
func _process(_delta):
	...
	if _isDraggingControl:
		_control.rect_global_position = globalMousePosition - _localMousePosition

# Pass in a displayOffset for fine tuning the popup position
func ShowPopupMenu(popupOffset):
	_popupMenu.rect_position = popupOffset
...
func _on_Control_gui_input(event):
	if event is InputEventMouseButton and event.button_index == BUTTON_LEFT and event.is_pressed():
		...
		var controlPosition = _control.rect_global_position
...
func _on_PopupMenuIcon_pressed():
	if _popupMenu.visible:
		...
		var popupOffset = _control.rect_global_position + Vector2(90, 2)
		ShowPopupMenu(popupOffset)
